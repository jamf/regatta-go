...
func (info TLSInfo) baseConfig() (*tls.Config, error) {
	...
	cfg.GetCertificate = func(clientHello *tls.ClientHelloInfo) (cert *tls.Certificate, err error) {
		...
		if os.IsNotExist(err) {
			if info.Logger != nil {
				info.Logger.Warnf(
					"failed to find peer cert files cert: %s key: %s err: %v", info.CertFile, info.KeyFile, err)
			}
		} else if err != nil {
			if info.Logger != nil {
				info.Logger.Warnf("failed to create peer certificate cert=%s key=%s %v", info.CertFile, info.KeyFile, err)
			}
		}
		return cert, err
	}
	cfg.GetClientCertificate = func(unused *tls.CertificateRequestInfo) (cert *tls.Certificate, err error) {
		...
		if os.IsNotExist(err) {
			if info.Logger != nil {
				info.Logger.Warnf("failed to find client cert files cert=%s key=%s %v", info.CertFile, info.KeyFile, err)
			}
		} else if err != nil {
			if info.Logger != nil {
				info.Logger.Warnf("failed to create client certificate cert=%s key=%s %v", info.CertFile, info.KeyFile, err)
			}
		}
		return cert, err
	}
...
func (info TLSInfo) ClientConfig() (*tls.Config, error) {
	...
	if !info.Empty() {
		...
		cfg = &tls.Config{ServerName: info.ServerName, MinVersion: tls.VersionTLS12, MaxVersion: tls.VersionTLS13}
	}
